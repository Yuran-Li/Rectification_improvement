...
def answer_corrected_match(gold_answer, output_answer):
    
    gold_answer = gold_answer.replace(" ", "")
    output_answer = output_answer.replace(" ", "")
    
    if gold_answer == output_answer:
        return True
    
    try:
        if "\\frac" in gold_answer and not gold_answer.endswith("}"):
            leaveout = gold_answer.split("}")[-1]
            resolved_answer = "}".join(gold_answer.split("}")[:-1]) + "}" + "{" + leaveout + "}"
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
            
        if "\\frac" in gold_answer:
            try:
                denominator, numerator = map(int, re.findall(r"\\frac{(\d+)}{(\d+)}", gold_answer)[0])
                if denominator / numerator == eval(output_answer):
                    # print(f"Resolved: {gold_answer}, {output_answer}")
                    return True
            except:
                pass
        ...
        if "_{" in gold_answer:
            resolved_answer = gold_answer.replace("{", "").replace("}", "")
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
        
        # "extracted_answers": ["\\pm1,\\pm2,\\pm3,\\pm6"], "gold_extracted_answer": "-6, -3, -2, -1, 1, 2, 3, 6"}
        if "\\pm" in gold_answer and "\\pm" not in output_answer:
            pattern = re.compile(r"\\pm(\d+)")
            pm_answers = pattern.findall(gold_answer)
            resolved_answer = gold_answer
            for pm_answer in pm_answers:
                if pm_answer not in output_answer:
                    resolved_answer = resolved_answer.replace(f"\\pm{pm_answer}", f"{pm_answer}, {-pm_answer}")
            gold_answer = resolved_answer
        
        if "," in gold_answer:
            resolved_answer = gold_answer.replace(",", "")
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
        
            answer_set = set([a.strip() for a in gold_answer.split(",")])
            output_set = set([a.strip() for a in output_answer.split(",")])
            if answer_set == output_set:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
        ...
        if pattern.match(gold_answer):
            resolved_answer = pattern.match(gold_answer).group(1)
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
        
        if "\\left" in gold_answer or "\\right" in gold_answer:
            resolved_answer = gold_answer.replace("\\left", "").replace("\\right", "")
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
            
        if "\sqrt" in gold_answer:
            resolved_answer = gold_answer.replace("{", "").replace("}", "")
            resolved_output_answer = output_answer.replace("{", "").replace("}", "")
            if resolved_answer == resolved_output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True
        
        # "extracted_answers": ["31518", "334650", "335670"], "gold_extracted_answer": "335{,}670"}
        if "{,}" in gold_answer:
            resolved_answer = gold_answer.replace("{,}", "")
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
        ...
        if "dfrac" in gold_answer:
            resolved_answer = gold_answer.replace("dfrac", "frac")
            if resolved_answer == output_answer:
                # print(f"Resolved: {gold_answer}, {output_answer}")
                return True

        if "+" in gold_answer:
            resolved_answer = set(gold_answer.split("+"))
            output_set = set(output_answer.split("+"))
            if resolved_answer == output_set:
                return True
...
if __name__ == "__main__":
    ...
    b = "(3,\\frac{\\pi}{2})"
    print(answer_corrected_match(a, b))  