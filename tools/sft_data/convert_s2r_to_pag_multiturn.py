...
# Same strings as PAG RL (vllm_pag_rollout_spmd.py ← pag_prompts.py)
VERIFY_TEMPLATE = VERIFY_USER
REGENERATE_TEMPLATE = REGENERATE_USER
SYSTEM = (
...
def normalize_verify(verify: str, verdict: str) -> str:
    ...
    # Luna often boxes the gold in verify; PAG forbids \\boxed{} here
    text = re.sub(r"\\boxed\{[^{}]*\}", "", text)
    text = re.sub(r"\\\[\s*\\\]", "", text)
    # Strip leading/trailing closers (incl. "The answer **4 is correct**.")
    text = re.sub(
        r"(?:^|\n)\s*The answer\s*(?:\*\*[^*]+\*\*\s*)?is\s+(?:incorrect|correct|wrong)\.?\s*",
        "\n",
        text,
        flags=re.I,
    )
...
def detect_verdict(verify: str) -> str:
    ...
    if "cannot verify" in vl:
        return "cannot"
    m = re.search(r"the answer\b.{0,80}?\bis\s+(incorrect|wrong|correct)\b", vl, re.S)
    if m:
        g = m.group(1)
        return "incorrect" if g in ("incorrect", "wrong") else "correct"
...
def main() -> None:
    ...
    ap.add_argument(
        "--out_dir",
        type=Path,
        default=Path("datasets/sft/Qwen-7B"),
    )
    ...
    ap.add_argument(
        ...
        default=1.0,
        help=(
            "Target ratio correct_verify / incorrect_verify in mixed split. "
            "e.g. 1.0 = 1:1, 0.5 = half as many correct as incorrect (default=1.0 off). "
            "Set to >0 to upsample correct-path verify rows in the mixed split only."
        ),
    )
    args = ap.parse_args()
    ...
    # Optionally upsample correct-path verify rows in the mixed split so the
    # verifier sees a more balanced correct/incorrect signal during SFT.
    correct_verify = [r for r in verify_rows if r.get("verdict") in ("correct", "unknown")]
    incorrect_verify = [r for r in verify_rows if r not in correct_verify]
    if args.upsample_correct > 0 and correct_verify and incorrect_verify:
        target_n = int(round(len(incorrect_verify) * args.upsample_correct))
        rng_up = random.Random(args.seed + 1)
        if target_n > len(correct_verify):
            extra = target_n - len(correct_verify)
            upsampled = correct_verify + [
                rng_up.choice(correct_verify) for _ in range(extra)
            ]
        else:
            upsampled = rng_up.sample(correct_verify, target_n)
        balanced_verify = incorrect_verify + upsampled
    else:
        balanced_verify = verify_rows

    mixed_rows = balanced_verify + rectify_rows
    ...
    stats = {
        ...
        "n_correct_verify_in_mixed": len([r for r in balanced_verify if r.get("verdict") in ("correct","unknown")]),
        "n_incorrect_verify_in_mixed": len([r for r in balanced_verify if r.get("verdict") not in ("correct","unknown")]),
        "upsample_correct": args.upsample_correct,
        "verdict_counts": {},
        "path_counts": {},
        "errors": errors[:20],
        "templates": {
